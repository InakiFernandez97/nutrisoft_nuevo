...
if(isset($_GET['id'])){
  ...
  $productos = menuControlador::viewMenuID($_GET['id']);
  foreach($productos as $re) {
  ...
  $men2 = menuControlador::todosMenu2($id);

}else{
 
  echo  '<script>window.location="AgregoIngredientes"</script>'; 
}
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"> Ficha <small>Menú</small></h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
    ...
    <div class="content">
      <div class="container-fluid">
        <div class="row">
            ...
            <div class="card card-primary">
              ...
              <form id="formulario">
                ...
                <div class="form-group">
                  <label for="exampleSelectRounded0">Menú<b> Seleccionado</b></label>
                  <input type="numero" class="form-control" id="men"  name="men"  value="<?=$nombre ?>" readonly>
                  <input type="hidden" class="form-control" id="pro"  name="pro"  value="<?=$id ?>" >
                  </div>
                  <div class="form-group">
          ...
          <div class="card card-purple card-outline">
         <div class="card-header">
           <h5 class="card-title m-0">Ingredientes el Menú</h5>
         </div>
         <div class="card-body">
            <!-- general form elements -->
       <div class="card card-primary">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php 
                   ...
                   foreach($men2 as $men) {?>
                  <tr>
                  <td><?php echo $men->Id_Producto; ?></td>
                    <td><?php echo $men->Nombre_Producto; ?></td>
                    <td><?php echo $men->gramaje; ?></td>
                    <td><?php echo $men->carbohidrato; ?></td>
                    <td><?php echo $men->calorias; ?></td>
                    <td><?php echo $men->proteinas; ?></td>
                    <td><?php echo $men->comentario; ?></td>
                    <td>    <button type="button" name="borrar" id="<?php echo $men->id_detalle; ?>" class="btn btn-danger btn-xs rojo borrar">Borrar</button></td>
...
<script>
  ...
  <script>
    ...
    $("#ErroresRegistro").html('');
    e.preventDefault(); // Detienes la accion por defecto
    var menu =    $("#pro").val();// obtienes el valor del input
  ...
  $(document).on('click', '.borrar', function(){
    ...
    var me = "FichaMenu?id="+menu;
   // alert(me);
                if(confirm("Esta seguro de borrar el Ingrediente?: " + categoria))
                {
                    $.ajax({
                        url:"ajax/productosAjax.php",
                        method:"POST",
                        data:{id:categoria, 'deleteIngrediente':deleteIngrediente},
                        dataType : 'json',
                        success:function(response)
                        {
                          if(response.estado == 1){
